...
pub fn run(subtask: Option<&str>) {
    match subtask {
        ...
        Some("4") => subtask4::run(),
        Some(other) => {
            eprintln!("Invalid subtask for module 4: {other}");
            print_usage();
        }
...
fn run_all_subtasks() {
    println!("Module 4 - Running all subtasks");
    println!("-------------------------------");
...
fn print_usage() {
    println!("Run module 4 subtasks with:");
    println!("  cargo run -- 4 1");
    println!("  cargo run -- 4 2");
    println!("  cargo run -- 4 3");
    println!("  cargo run -- 4 4");
}
